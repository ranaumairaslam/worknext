...
router.put('/:projectId', authorizeRole('company', 'team_leader'), async (req, res, next) => {
  try {
    const projectId = positiveInteger(req.params.projectId, 0, 1000000000);
    if (!projectId) {
      return res.status(400).json({ success: false, message: 'Invalid project id' });
...
router.patch('/:projectId/status', authorizeRole('company', 'team_leader'), async (req, res, next) => {
  try {
    const projectId = positiveInteger(req.params.projectId, 0, 1000000000);
